...
def get_points_beta_corrected(rel_angle, exp_angle, points):
    ...
    f_beta = exp_angle - rel_angle
    v_beta = - (f_beta / len(points))  # 注意公式！这里是负的
    log(f"f_β={f_beta}, v_β = {v_beta}")
    new_points = copy.deepcopy(points)
    for point in new_points:
        log(f"正在修正点{point[PointDataKeys.name]}的β：{point[PointDataKeys.beta_angle]}")
        point[PointDataKeys.beta_angle] += Angle(str(v_beta))
        log(f"修正为{point[PointDataKeys.beta_angle]}")
    # log(f"函数中返回的平差后的数据{new_points}")
    return new_points
    ...
    if traverse_data.get(MeasureDataKeys.measureType) != MeasureType.ConnectingTraverse:
        ...
        for i in range(len(exp_data_points)):
            ...
            if i != len(exp_data_points) - 1:
                ...
                log(f"确定当前的点上一条线的alpha为：{previous_line_alpha}")
                ...
                log(f"准备坐标正算，坐标{pos},下个alpha{coming_line_alpha},下段线{coming_line[LineDataKeys.name]}长度为{next_line_length}")
                ...
                log(f"写入后的next_point是否保持一致，1-4没有坐标？{next_point}")


                log(f"坐标正算结果：下一条线{"".join(coming_line[LineDataKeys.component_points])}的alpha为{coming_line_alpha}")
                log(f"下一个点{next_point[PointDataKeys.name]}坐标为{next_point[PointDataKeys.pos]}")

        log(f"初步计算结果：{exp_data_points}")
        rel_end_point = traverse_data[MeasureDataKeys.points][-1]
        exp_end_point = exp_data_points[-1]
        log(f"预测的C点：{exp_end_point} \n实际的C点：{rel_end_point}")
        f_x, f_y = get_fx_fy(rel_end_point[PointDataKeys.pos], exp_end_point[PointDataKeys.pos])
        log(f"fx {f_x}  f_y {f_y}")

        total_distance = total_D(exp_data_lines)
        log(f"全长{total_distance}")
...
def connectionTraverse_calculate_v2(traverse_data: dict, wanted_accuracy: int):
    ...
    if 1 + 1 == 3:  #traverse_data.get(MeasureDataKeys.measureType) != MeasureType.ConnectingTraverse:
        ...
        log(f"初始的点数据，夹角未平差{beta_correction_points_data}")
        ...
        log(f"平差后的点数据{beta_correction_points_data}")
        ...
        for i in range(len(corrected_points)):
            ...
            if i != len(corrected_points) - 1:
                ...
                beta = point[PointDataKeys.beta_angle]
                beta_direction = point[PointDataKeys.beta_angle_direction]
                log(f"准备分析，内容为：beta {beta}  beta_direction  {beta_direction}")
                ...
                log(f"在该点处的坐标正算结果是，Δ'x ={delta_x}， Δ'y ={delta_y}")
                total_delta_x += delta_x
                total_delta_y += delta_y
                delta_data.append([delta_x, delta_y])
        log(f"完成坐标正算，ΣΔx' = {total_delta_x}, ΣΔy' = {total_delta_y}")
        ...
        for i in range(len(corrected_points)):
            point = corrected_points[i]
            if i != len(corrected_points) - 1:
                log(f"对第{i}点{point[PointDataKeys.name]}进行分析，计算下一点。")
                pos = point[PointDataKeys.pos]
                ...
                log(f"即对{coming_line[LineDataKeys.name]}进行处理")
                ...
                log(f"下一点{next_point[PointDataKeys.name]}平差后坐标为{next_point_pos}")
...
def connectionTraverse_calculate_V3(traverse_data: dict, wanted_accuracy: int):
    ...
    for i in range(len(raw_points)):
        ...
        # log("信息：")
        # log(traverse_data[MeasureDataKeys.lines])
        ...
        log(f"此处{point[PointDataKeys.name]}点下一段线的α修正为{alpha} (此处beta{beta_here}修正为{corrected_beta_here})")
    ...
    for line in raw_lines:
        ...
        log(f"对于线段{name}, alpha={alpha}, Δx' = {delta_x_rough}, Δy'= {delta_y_rough}")
    log(f"总的结果ΣΔx'、ΣΔy'为{total_delta_rough}\n")
    ...
    log(f"计算出f_x {f_x} f_y {f_y}\n")

    total_length = total_D(raw_lines)
    log(f"总长度为{total_length}")
    for i in range(len(raw_lines)):
        ...
        log(f"修正后的Δx_{name} = {delta_x_true} Δy_{name} = {delta_y_true}")
        ...
        if i != len(raw_lines) - 1:  # 不是最后一条线段才写入，最后一条线段的话，结束点是已知的
            ...
            log(f"此处的点{end_point_of_this_line[PointDataKeys.name]}的坐标为：{[x_here, y_here]}")
